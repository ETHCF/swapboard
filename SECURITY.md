...
7. **Caller payouts can be redirected; maker tokenB cannot**: Fill, cancel, and modify take `recipient`. `address(0)` pays `msg.sender`. Any other address receives the caller's ETH and tokens (tokenA on fill, tokenA refunds on cancel and modify). A caller that rejects ETH, or cannot hold the ERC20, can name an address that can. TokenB still goes to the maker, so a maker that rejects ETH still cannot be paid ETH tokenB, and that order stays unfillable.
